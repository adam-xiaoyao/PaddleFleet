...
// ==========================================================================
// Clamped variants (with clamp_value parameter)
// ==========================================================================

// ---- Clamped Forward Kernel ----
template <typename T, typename ScaleT, int VEC_SIZE>
__global__ void VectorizedFusedSwiGLUClampFwd(const T* __restrict__ x,
                                              const ScaleT* __restrict__ scale,
                                              T* __restrict__ out,
                                              int hidden_size,
                                              int row_stride,
...
// ---- Clamped Backward Kernel ----
template <typename T, typename ScaleT, int VEC_SIZE>
__global__ void VectorizedFusedSwiGLUClampBwd(const T* __restrict__ x,
                                              const ScaleT* __restrict__ scale,
                                              const T* __restrict__ d_out,
                                              T* __restrict__ d_x,
                                              ScaleT* __restrict__ d_scale,
...
std::vector<paddle::Tensor> FusedSwiGLUScaleClampForward(
  ...
  PADDLE_ENFORCE_LE(
      ...
      static_cast<int64_t>(std::numeric_limits<int>::max()),
      common::errors::InvalidArgument(
          "rows * hidden2 must be <= INT_MAX for fused_swiglu_scale_clamp."));
  ...
  if (x.dtype() == paddle::DataType::BFLOAT16) {
    ...
    using cuda_bf16 = __nv_bfloat16;
    if (scale.dtype() == paddle::DataType::FLOAT32) {
      VectorizedFusedSwiGLUClampFwd<cuda_bf16, float, 8>
          <<<grid_size, block_size, 0, stream>>>(
              reinterpret_cast<const cuda_bf16*>(x.data<paddle_bf16>()),
              scale.data<float>(),
              reinterpret_cast<cuda_bf16*>(out.data<paddle_bf16>()),
      ...
      VectorizedFusedSwiGLUClampFwd<cuda_bf16, cuda_bf16, 8>
          <<<grid_size, block_size, 0, stream>>>(
              reinterpret_cast<const cuda_bf16*>(x.data<paddle_bf16>()),
              reinterpret_cast<const cuda_bf16*>(scale.data<paddle_bf16>()),
              reinterpret_cast<cuda_bf16*>(out.data<paddle_bf16>()),
    ...
    VectorizedFusedSwiGLUClampFwd<float, float, 4>
        <<<grid_size, block_size, 0, stream>>>(x.data<float>(),
                                               scale.data<float>(),
                                               out.data<float>(),
                                               hidden_size,
...
std::vector<paddle::Tensor> FusedSwiGLUScaleClampBackward(
  ...
  if (x.dtype() == paddle::DataType::BFLOAT16) {
    ...
    using cuda_bf16 = __nv_bfloat16;
    if (scale.dtype() == paddle::DataType::FLOAT32) {
      VectorizedFusedSwiGLUClampBwd<cuda_bf16, float, 8>
          <<<grid_size, block_size, 0, stream>>>(
              reinterpret_cast<const cuda_bf16*>(x.data<paddle_bf16>()),
              scale.data<float>(),
              reinterpret_cast<const cuda_bf16*>(d_out.data<paddle_bf16>()),
      ...
      VectorizedFusedSwiGLUClampBwd<cuda_bf16, cuda_bf16, 8>
          <<<grid_size, block_size, 0, stream>>>(
              reinterpret_cast<const cuda_bf16*>(x.data<paddle_bf16>()),
              reinterpret_cast<const cuda_bf16*>(scale.data<paddle_bf16>()),
              reinterpret_cast<const cuda_bf16*>(d_out.data<paddle_bf16>()),
    ...
    VectorizedFusedSwiGLUClampBwd<float, float, 4>
        <<<grid_size, block_size, 0, stream>>>(x.data<float>(),
                                               scale.data<float>(),
                                               d_out.data<float>(),
                                               d_x.data<float>(),
